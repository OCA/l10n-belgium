...
class TestCodaFile(TransactionCase):
    ...
    def setUp(self):
        ...
        self.statement_model = self.env["account.statement"]
    ...
    def test_coda_file_import(self):
        ...
        bank_st_record = self.statement_model.search([("name", "=", "2012/135")])[
            0
        ]
        self.assertEqual(
    ...
    def test_coda_file_import_twice(self):
        statement_import = self.statement_import_model.create(
            {
                "attachment_ids": [
                    (0, 0, {"name": "coda test file", "datas": self.coda_file})
                ]
    ...
    def test_coda_file_wrong_journal(self):
        ...
        with self.assertRaises(Exception):
            ...
            statement_import.with_context(journal_id=cash_journal.id).import_file()
