...
class TestVatListing(TestVatReportsCommon):
    ...
    def _prepare_listing(self):
        ...
        vat_listing = next(vl for vl in vat_listings if vl.vat == self.partner.vat)
        ...
        full_list = self.env["partner.vat.list"].with_context(context).create({})
        return full_list
    ...
    @unittest.skipIf(report.wkhtmltopdf_state == "install", "wkhtmltopdf not available")
